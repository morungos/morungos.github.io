...
We aren't strict at testing, but when modernizing an application, it has been our
experience that having something like Puppeteer can be extremely useful, as it 
...
will also conveniently serve static files, which `php-fpm` never does. Also, while environment
variables typically do show in the requests, the `$_SERVER` variable is very 
different. So, if you test using `php` and deploy using `php-fpm`, there are 
some significant risks that your application will break in production.
...
server that wraps around `php-fpm`, and that is exactly consistent with the way
we run in production. 

Our server script, now [on Github as a gist](https://gist.github.com/morungos/f468e00dfb20d63f6ea9300fdc92df43), lets you do just
...
What's odd, using the Slim framework for PHP, we could confirm that the written
body size was 5 octets, so we know that what we are sending out of PHP is correct, 
but that what arrives in the client is 7 octets. In fact, every single character
...
I finally twigged that our test system was *not transparent* -- there was essentially
...
In other words, my own test tool had been so perfectly transparent that I had 
completely forgotten that it even existed, and yet, it was the fault all along.

Anyway, I digress: it turns out that the character encoding problem was in 
...
look in the raw *buffer* for CRLF-CRLF (not a decoded string), and only then translate 
...
* PHP's character encodings suck. When you look into the documentation, it's 
